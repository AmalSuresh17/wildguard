...
def adminmanage_foreststation(request):
    ...
    return render(request,"adminmanage_foreststation.html",{'q1':q1,'qa':qa})
...
def preserved_remove(request,id):
    ...
    qb.delete()
    return HttpResponse("<script>alert('Removed Successfully');window.location='/adminmanage_panimals'</script>")
...
def update_officer(request,id):
    qa=forest_officer.objects.all()
    q1=forest_officer.objects.get(forest_officer_id=id)
    if'update'in request.POST:
        fname=request.POST['fname']
        lname=request.POST['lname']
        place=request.POST['place']
...
def reply(request,id):
    ...
    if 'reply' in request.POST:
        ...
        qa.reply=reply
        qa.save()
        return HttpResponse("<script>alert('Replied Successfully');window.location='/admincomplaint'</script>")
